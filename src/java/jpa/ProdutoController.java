...
public class ProdutoController implements Serializable {
    ...
    @EJB
    private jpa.ProdutoFacade ejbFacade;
    ...
    public Produto getSelected() {
        if (current == null) {
            current = new Produto();
            current.setProdutoPK(new jpa.ProdutoPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public String create() {
        try {
            current.getProdutoPK().setExpositorId(current.getExpositor().getId());
            current.getProdutoPK().setEstandeId(current.getEstande().getId());
    ...
    public static class ProdutoControllerConverter implements Converter {
        ...
        jpa.ProdutoPK getKey(String value) {
            jpa.ProdutoPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new jpa.ProdutoPK();
            key.setId(Integer.parseInt(values[0]));
            key.setExpositorId(Integer.parseInt(values[1]));
            key.setEstandeId(Integer.parseInt(values[2]));
            return key;
        }

        String getStringKey(jpa.ProdutoPK value) {
            StringBuilder sb = new StringBuilder();
            sb.append(value.getId());
            sb.append(SEPARATOR);
            sb.append(value.getExpositorId());
            sb.append(SEPARATOR);
            sb.append(value.getEstandeId());
            return sb.toString();
        }
        ...
        public String getAsString(FacesContext facesContext, UIComponent component, Object object) {
            ...
            if (object instanceof Produto) {
                ...
                return getStringKey(o.getProdutoPK());
            } else {
